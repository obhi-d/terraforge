...
layout(local_size_x = 1) in;
  ...
  layout(std430, binding = data_buffer_b) restrict readonly buffer data_buffer {
    vec2 minmax;
  }u_data;
  ...
  void main()
  {   
    float value = (imageLoad(source, ivec2(gl_GlobalInvocationID.xy)).x -  u_data.minmax.x) / (u_data.minmax.y - u_data.minmax.x);
    imageStore(source, ivec2(gl_GlobalInvocationID.xy), vec4(value * 2.0 - 1.0));
  }
