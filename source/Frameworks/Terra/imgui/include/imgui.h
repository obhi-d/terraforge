...
namespace ImGui
{
    ...
    IMGUI_API void          MaximizeWindow();
    IMGUI_API void          RestoreWindow();
    // Window manipulation
...
enum ImGuiColorEditFlags_
{
    ...
    ImGuiColorEditFlags_Uint8           = 1 << 23,  // [DataTypeEnum]   // ColorEdit, ColorPicker, ColorButton: _display_ values formatted as 0..255.
    ImGuiColorEditFlags_Float           = 1 << 24,  // [DataTypeEnum]   // ColorEdit, ColorPicker, ColorButton: _display_ values formatted as 0.0f..1.0f floats instead of 0..255 integers. No round-trip of value via integers.
...
struct ImGuiPayload
{
    ...
    char            DataTypeEnum[32 + 1];   // Data type tag (short user-supplied string, 32 characters max)
    bool            Preview;            // Set when AcceptDragDropPayload() was called and mouse has been hovering the target item (nb: handle overlapping drag targets)
    bool            Delivery;           // Set when AcceptDragDropPayload() was called and mouse button is released over the target item.

    ImGuiPayload()  { Clear(); }
    void Clear()    { SourceId = SourceParentId = 0; Data = NULL; DataSize = 0; memset(DataTypeEnum, 0, sizeof(DataTypeEnum)); DataFrameCount = -1; Preview = Delivery = false; }
    bool IsDataType(const char* type) const { return DataFrameCount != -1 && strcmp(type, DataTypeEnum) == 0; }
...
struct ImGuiPlatformIO
{
    ...
    void    (*Platform_Maximize)(ImGuiViewport* vp);
    void    (*Platform_Restore)(ImGuiViewport* vp);
