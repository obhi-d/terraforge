...
namespace terra
{
std::shared_ptr<ImguiTerraWindow> ImguiTerraContext::createWindow(std::string name, Magnum::Vector2i size,
                                                                  Magnum::Vector2i pos)
{
  auto window = std::make_shared<ImguiTerraWindow>(*this, name, size, pos);
  windowMap.emplace(window->getId(), (uint32_t)windows.size());
