...
ImLine ed::Pin::GetClosestLine(const Pin* pin) const
{
    ...
    if (pin->SnapLinkToDir() && extentB > 0.0f)
    {
...
void ed::Link::Draw(ImDrawList* drawList, ImU32 color, float extraThickness) const
{
    ...
    ImDrawList_AddBezierWithArrows(drawList, curve, m_Thickness + extraThickness,
        ...
        true, color, 1.0f,
        m_StartPin && m_StartPin->SnapLinkToDir() ? &m_StartPin->m_Dir : nullptr,
        m_EndPin   &&   m_EndPin->SnapLinkToDir() ?   &m_EndPin->m_Dir : nullptr);
}
...
bool ed::EditorContext::Begin(const char* id, const ImVec2& size)
{
    ...
    if (availableContentSize.x <= 0.0f || availableContentSize.y <= 0.0f)
      return false;
    ...
    static auto resetAndCollect = [](auto& objects)
    {
        ...
        {
            if (objectWrapper->DeleteOnNewFrame())
            {
...
void ed::EditorContext::End()
{
    ...
    // Highlight selected objects
    {
        ...
        {
            return pin.m_Node->HighlightConnectedLinks() && pin.m_Node->IsSelected();
        };
...
# if 1
    ...
    {
        // Copy group nodes
        auto liveNodeCount = static_cast<int>(std::count_if(m_Nodes.begin(), m_Nodes.end(), [](Node* node) { return node->IsLive(); }));
...
bool ed::EditorContext::DoLink(LinkId id, PinId startPinId, PinId endPinId, ImU32 color, float thickness)
{
    ...
    startPin->HasConnection() = true;
      endPin->HasConnection() = true;
...
void ed::EditorContext::PushClipRect(const ImVec2& min, const ImVec2& max) 
{
  m_DrawList->PushClipRect(min, max, false);
}

void ed::EditorContext::PopClipRect() 
{
...
{
    bool tryLoadState = node->RestoreState();

    node->RestoreState() = false;
...
bool ed::EditorContext::PinHadAnyLinks(PinId pinId)
{
    ...
    return pin->HasConnection() || pin->HadConnection();
}
...
void ed::EditorContext::SaveSettings()
{
    ...
    for (auto& node : m_Nodes)
    {
        ...
        if (!node->RestoreState() && settings->m_IsDirty && m_Config.SaveNodeSettings)
        {
...
ed::Control ed::EditorContext::BuildControl(bool allowOffscreen)
{
    ...
    ImGuiButtonFlags extraFlags = ImGuiButtonFlags_PressedOnClickRelease;
    ...
    for (auto nodeIt = m_Nodes.rbegin(), nodeItEnd = m_Nodes.rend(); nodeIt != nodeItEnd; ++nodeIt)
    {
        ...
        for (auto pin = node->m_LastPin; pin; pin = pin->m_PreviousPin)
        {
            ...
            if (pin->ExplicitInteractions())
            {
              if (pin->IsClicked())
                clickedObject = pin;
              if (!doubleClickedObject && pin->IsDoubleClicked())
                doubleClickedObject = pin;
              if (pin->IsHovered())
                hotObject = pin;
              if (pin->IsActive())
              {
        ...
        {
          if (node->ExplicitInteractions())
          {
            if (node->IsClicked())
              clickedObject = node;
            if (!doubleClickedObject && node->IsDoubleClicked())
              doubleClickedObject = node;
            if (node->IsHovered())
              hotObject = node;
            if (node->IsActive())
              activeObject = node;
...
void ed::NodeBuilder::SetObjectFlags(Object* object, ImneObjFlags flags, bool state)
{
  if (flags & ImneObjFlags::ImneObjFlags_ExplicitInteractions)
    object->ExplicitInteractions() = state;
  if (object->ExplicitInteractions())
  {
    if (flags & ImneObjFlags::ImneObjFlags_IsActive)
      object->IsActive() = state;
    if (flags & ImneObjFlags::ImneObjFlags_IsDoubleClicked)
      object->IsDoubleClicked() = state;
    if (flags & ImneObjFlags::ImneObjFlags_IsClicked)
      object->IsDoubleClicked() = state;
    if (flags & ImneObjFlags::ImneObjFlags_IsHovered)
      object->IsHovered() = state;
  }
}

void ed::NodeBuilder::SetInteractionFlags(Object* object, ImneObjFlags flags)
{
  if (object->ExplicitInteractions())
  {
    object->IsActive()        = (flags & ImneObjFlags::ImneObjFlags_IsActive);
    object->IsDoubleClicked() = (flags & ImneObjFlags::ImneObjFlags_IsDoubleClicked);
    object->IsClicked()       = (flags & ImneObjFlags::ImneObjFlags_IsClicked);
    object->IsHovered()       = (flags & ImneObjFlags::ImneObjFlags_IsHovered);
  }
}
...
void ed::NodeBuilder::SetPinFlags(PinId nodeId, PinKind kind, ImneObjFlags flags, bool state)
{
  auto pin = Editor->GetPin(nodeId, kind);
  if (pin)
    SetObjectFlags(pin, flags, state);
}

void ed::NodeBuilder::SetCurrentPinFlags(ImneObjFlags flags, bool state)
{
...
void ed::NodeBuilder::SetPinInteraction(ImneObjFlags flags)
{
  auto pin = m_CurrentPin;
  if (pin)
...
void ed::NodeBuilder::Begin(NodeId nodeId)
{
    ...
    if (m_CurrentNode->CenterOnScreen())
    {
        ...
        m_CurrentNode->CenterOnScreen() = false;
    }
    ...
    m_CurrentNode->HighlightConnectedLinks() = editorStyle.HighlightConnectedLinks != 0.0f;
...
void ed::NodeBuilder::BeginPin(PinId pinId, PinKind kind)
{
    ...
    m_CurrentPin->SnapLinkToDir() = editorStyle.SnapLinkToPinDir != 0.0f;
