...
struct Object
{
    ...
    enum Flags : uint32_t
    {
      ...
      eIsClicked,
      eIsHovered,
      eExplicitInteractions,
      eSnapLinkToDir,
      eHasConnection,
      eHadConnection,
      eHighlightConnectedLinks,
      eRestoreState,
      eCenterOnScreen
    };
    ...
    {
      uint32_t& src;
      FlagAccessor(uint32_t const& on) : src(const_cast<uint32_t&>(on)) {}
      inline FlagAccessor& operator=(bool value) 
      {
        if (value)
          src |= (1u << flag);
        else
    ...
    #define FLAG_ACCESSOR(Flag) FlagAccessor<e##Flag> Flag() const { return FlagAccessor<e##Flag>(m_Flags); }

    EditorContext* const Editor;
    uint32_t             m_Flags = (1 << eIsLive);

    FLAG_ACCESSOR(IsLive)
    FLAG_ACCESSOR(IsSelected)
    FLAG_ACCESSOR(DeleteOnNewFrame)
    FLAG_ACCESSOR(IsActive)
    FLAG_ACCESSOR(IsDoubleClicked)
    FLAG_ACCESSOR(IsClicked)
    FLAG_ACCESSOR(IsHovered)
    FLAG_ACCESSOR(ExplicitInteractions)
...
struct Pin final: Object
{
    ...
    float   m_ArrowSize;
    float   m_ArrowWidth;
    FLAG_ACCESSOR(SnapLinkToDir)
    FLAG_ACCESSOR(HasConnection)
    FLAG_ACCESSOR(HadConnection)
...
struct Node final: Object
{
    ...
    FLAG_ACCESSOR(HighlightConnectedLinks)
    FLAG_ACCESSOR(RestoreState)
    FLAG_ACCESSOR(CenterOnScreen)
...
struct NodeBuilder
{
    ...
    void SetNodeFlags(NodeId id, ImneObjFlags flags, bool state);
    void SetPinFlags(PinId id, PinKind kind, ImneObjFlags flags, bool state);
...
struct EditorContext
{
    ...
    void PushClipRect(const ImVec2& min, const ImVec2& max);
    void PopClipRect();
